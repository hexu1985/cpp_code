...
PropertiesDialog::PropertiesDialog(Node *node,
    const Undirected_graph::vertex_type &v,
    QWidget *parent) : QDialog(parent)
{
    ...
    vertexIdLabel->setText(QString::number(node->id()));
    if (v.parent < 0) {
        parentIdLabel->setText("NIL");
    } else {
        parentIdLabel->setText(QString::number(v.parent));
    }

    if (v.distance < 0) {
        distanceLabel->setText("infinity");
    } else {
        distanceLabel->setText(QString::number(v.distance));
    }
}
