...
{
    Node *node = new Node(seqNumber+1);
...
void DiagramWindow::properties()
{
    ...
    if (node) {
        PropertiesDialog dialog(node, graph.get_vertex(node->id()), this);
        dialog.exec();
    } else if (link) {
...
void DiagramWindow::updateActions()
{
    ...
    if (bfsAction->isEnabled()) {
        view->addAction(separatorAction);
        view->addAction(bfsAction);
    }
}
...
void DiagramWindow::createMenus()
{
    ...
    algoMenu = menuBar()->addMenu(tr("&Algo"));
