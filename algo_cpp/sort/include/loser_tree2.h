#ifndef	HX_LOSER_TREE2_H
#define	HX_LOSER_TREE2_H
...
private:
	...
	std::vector<Sequence_base *> sequences_;	// 败者树一个叶子节点, 对应一路归并序列
	int ninners_;
	int nleaves_;
	Compare compare_;
...
public:
	...
	int competition(Loser_info &parent_loser, int node_index) {
		...
		Sequence_base *parent_seq = sequence(parent_loser.index);
		Sequence_base *seq = sequence(node_index);
