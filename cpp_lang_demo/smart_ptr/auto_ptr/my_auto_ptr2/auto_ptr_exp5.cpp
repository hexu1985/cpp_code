...
int main () {
  ...
  Auto_ptr<int> p;   // uses default deleter

  // alpha and beta use independent copies of the deleter:
  Auto_ptr<int,state_deleter> alpha (new int);
  Auto_ptr<int,state_deleter> beta (new int,alpha.get_deleter());

  // gamma and delta share the deleter "del" (deleter type is a reference!):
  Auto_ptr<int,state_deleter&> gamma (new int,del);
  Auto_ptr<int,state_deleter&> delta (new int,gamma.get_deleter());
