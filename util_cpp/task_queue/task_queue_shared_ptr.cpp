...
class Task_queue: public std::queue<std::shared_ptr<Task_base>> {
...
public:
	...
	void swap_task(std::queue<std::shared_ptr<Task_base>> &task_queue) {
		std::lock_guard<std::mutex> lck(queue_mtx_);
		if (!this->empty()) {
...
{
	std::queue<std::shared_ptr<Task_base>> work_queue;
	while (true) {
		while (!work_queue.empty()) {
			std::shared_ptr<Task_base> task = work_queue.front();
			work_queue.pop();
			task->run();
		}
