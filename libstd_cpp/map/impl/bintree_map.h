...
#ifndef HX_BINTREE_MAP_H
#define HX_BINTREE_MAP_H
 ...
 */
template <typename T>
struct map_node: public detail::bintree_node_t {
...
public:
	...
	mapped_type &operator [](const key_type &k)
	{
		...
		if (link == NULL) {	// k not in map
			...
			bintree_insert(&tree_, link, less_, get_key, hint);
		}
	...
	iterator erase(const_iterator position)
	{
		...
		++next;
		bintree_remove(&tree_, (link_type *) position.link);
	...
	std::pair<iterator, bool> emplace(Args &&... args)
	{
		...
		link_type *link = bintree_search(tree_.root, get_key(new_link), 
			less_, get_key, &hint);
		if (link == NULL) {
			bintree_insert(&tree_, new_link, less_, get_key, hint);
...
private:
	...
	struct move_clone {
		...
		link_type *operator ()(link_type *link)
		{
